...
$(document).ready(function () {
  var $card = $(".card"),
    $bgCard = $(".bgCard"),
    $icon = $(".icon"),
    cartPageBottom = $(".cart-page-bottom"),
    cartPageBottomH4 = cartPageBottom.find("h4"),
    cartPageBottomP = cartPageBottom.find("p");

  let textTitle = "Selamat Natal, Anglin temen gueee!";
  let charArrTitle = textTitle.split('');
  let text = "Semoga musim liburan ini ngasih kau kebahagiaan dan rezeki yang berlimpah, jangan lupa banyak makanan enak disana, jangan lupa juga potokan makananmu samaku apapun yang kau makan tapi jangan babi soalnya aku ngga bisa makan itu :), dan Nikmatin la setiap momen saat kumpul bareng sama keluarga dan teman-temanmu, dan biarkan tawa mengusir kesedihan dari hidupmu. Semoga tahun depan membawa berkah yang lebih besar, penuh canda tawa, dan kebahagiaan biar kau ngga perlu lagi ngerepost hal-hal sedih di TikTok mu :). Nikmati la liburanmu dengan penuh semangat dan kebahagiaan, tapi jangan lupakan laporan magangmu, ya wkwkwkwk :'). Lopyu...... Sekali lagi, Merry Christmas temankuuuuuuu! 🎄🎁🌟";
  ...
  function resetText() {
    ...
    cartPageBottomH4.text("");
    cartPageBottomP.text("");
    currentIndexTitle = 0;
    ...
    if ($card.hasClass("is-opened")) {
      textIntervalTitle = setInterval(function () {
        if (currentIndexTitle < charArrTitle.length) {
          cartPageBottomH4.text(cartPageBottomH4.text() + charArrTitle[currentIndexTitle]);
          currentIndexTitle++;
        } else {
          clearInterval(textIntervalTitle);
          textIntervalContent = setInterval(function () {
            if (currentIndexContent < charArrContent.length) {
              cartPageBottomP.text(cartPageBottomP.text() + charArrContent[currentIndexContent]);
              currentIndexContent++;
            } else {
              clearInterval(textIntervalContent);
            }
  ...
  $(".centerer").on("click", function () {
    ...
    $bgCard.fadeIn();
    $icon.fadeIn();
    updateTextSize(); // Update text size when the card is opened
  });
  ...
  // Function to update text size based on card dimensions
  function updateTextSize() {
    var cardWidth = $card.width();
    var cardHeight = $card.height();

    var fontSizeH4 = cardWidth * 0.065; // You can adjust the multiplier based on your preference
    var fontSizeP = cardWidth * 0.0452; // You can adjust the multiplier based on your preference

    cartPageBottomH4.css("font-size", fontSizeH4 + "px");
    cartPageBottomP.css("font-size", fontSizeP + "px");
  }
});
